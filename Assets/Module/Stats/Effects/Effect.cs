...
public class Effect : ScriptableObject
{
    ...
    public void ApplyOnUser (AttackableInfosPackage userInfo, AttackableInfosPackage targetInfo, AttackType attackType, int inflictedDamage, FightRegistry fightRegistry, Action onAnimeEnded)
    {
        ...
        _alteredValue = GetAlteredValue (userInfo.Stats, target.Stats);
        ApplyOnUserInternal (user, attack, target, inflictedDamage, fightRegistry, onAnimeEnded);
    }

    protected virtual void ApplyOnUserInternal (AttackableInfosPackage userInfo, AttackableInfosPackage targetInfo, AttackType attackType, int inflictedDamage, FightRegistry fightRegistry, Action onAnimeEnded)
    {
        onAnimeEnded?.Invoke ();
    }

    public void ApplyOnTarget (AttackableInfosPackage userInfo, AttackableInfosPackage targetInfo, AttackType attackType, int inflictedDamage, FightRegistry fightRegistry, Action onAnimeEnded)
    {
        if (!CheckApplyCondition (attackType, ApplyConditionsOnTarget))
        {
            onAnimeEnded?.Invoke ();
            return;
        }
    ...
    protected virtual void ApplyOnTargetInternal (AttackableInfosPackage userInfo, AttackableInfosPackage targetInfo, AttackType attackType, int inflictedDamage, FightRegistry fightRegistry, Action onAnimeEnded)
    {
        onAnimeEnded?.Invoke ();
    }

    private bool CheckApplyCondition (AttackType attackType, List<ApplyCondition> conditions)
    {
        bool apply = true;
        foreach (ApplyCondition condition in conditions)
        {
            switch (condition)
            {
                case ApplyCondition.AttackIsMelee:
                    apply &= attackType == AttackType.Melee;
                    break;
                case ApplyCondition.AttackIsProjectile:
                    apply &= attackType == AttackType.Projectile;
                    break;
            }
    ...
    protected static bool TargetHasTempEffect (AttackableInfosPackage targetInfo, TempEffect tmpEffect, TempEffect.Timeline timeline)
    {
        return targetInfo.tempEffects.ContainsKey (timeline) && targetInfo.tempEffects[timeline].Find (x => x.Name == tmpEffect.Name) != null;
    }
}
